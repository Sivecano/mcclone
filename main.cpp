...
int main() {
  ...
  win = SDL_CreateWindow("opengl with SDL2 :)", SDL_WINDOWPOS_CENTERED,
  ...
  while (running) {
    ...
    while (SDL_PollEvent(&e)) {
      ...
      case SDL_MOUSEMOTION:
        if (SDL_GetRelativeMouseMode() == SDL_TRUE) {
          ...
          if (playcam.pitch < -1.57)
            playcam.pitch = -1.57;
          playcam.direction = glm::normalize(glm::vec3(
              cos(playcam.pitch) * cos(playcam.yaw), sin(playcam.pitch),
      ...
      case SDL_MOUSEBUTTONUP:
        float dist =
            raycast_distance(playcam.position, playcam.direction, &world, 20.);
        if (dist < 20)
          switch (e.button.button) {
          case SDL_BUTTON_RIGHT:
            SDL_LogCritical(SDL_LOG_CATEGORY_APPLICATION, "adl");
            world.setBlock(
                glm::round(playcam.position + playcam.direction * (dist - 1)),
                hand);
            break;

          case SDL_BUTTON_LEFT:
            world.setBlock(
                glm::round(playcam.position + playcam.direction * (dist)), 0);
            break;
    ...
    float speed =
        (2 * keystate[SDL_SCANCODE_LCTRL] + 1) * (ctime - last) * 0.02f;

    playcam.position +=
        glm::normalize(glm::cross(glm::vec3(0, 1, 0),
                                  playcam.direction -
                                      glm::vec3(0, playcam.direction.y, 0))) *
        (speed * (keystate[SDL_SCANCODE_D] - keystate[SDL_SCANCODE_A]));
    playcam.position +=
        glm::vec3(0, 1, 0) * (speed * (keystate[SDL_SCANCODE_SPACE] -
                                       keystate[SDL_SCANCODE_LSHIFT]));
    playcam.position +=
        glm::normalize(playcam.direction -
                       glm::vec3(0, playcam.direction.y, 0)) *
        (speed * (keystate[SDL_SCANCODE_W] - keystate[SDL_SCANCODE_S]));

    playcam.FOV +=
        0.01f * (keystate[SDL_SCANCODE_E] - keystate[SDL_SCANCODE_Q]);
    playcam.FOV = fmin(3.14159f, playcam.FOV);
    playcam.FOV = fmax(0.1f, playcam.FOV);

    // SDL_Log("x: %f, y: %f, z: %f, fov: %f", playcam.position.x,
    // playcam.position.y, playcam.position.z, playcam.FOV);
    ...
    for (int i = -4; i < 5; i++)
      for (int j = -4; j < 5; j++)
        for (int k = -4; k < 4; k++)
          renderChunk(playcam, world.getChunk(glm::ivec3(
                                   i + floor(playcam.position.x / 16),
                                   k + floor(playcam.position.y / 16),
                                   j + floor(playcam.position.z / 16))));
