...
__device__ inline unsigned int bindex(unsigned int x, unsigned int y,
                                      unsigned int z) {
  return x + 16 * z + 256 * y;
}
// TODO: implement using the remaining two bytes as cross so we can have flowers
// and shit
/**
...
__global__ void calculate_cube_facemask(uint8_t *data) {
  ...
  if (x > 0)
    if (data[bindex(x - 1, y, z)] != 0)
      out &= ~16; // left
  if (y > 0)
    if (data[bindex(x, y - 1, z)] != 0)
      out &= ~1; // bottom
  if (z > 0)
    if (data[bindex(x, y, z - 1)] != 0)
      out &= ~2; // front // what happenes if u acess data out of chunk??????
                 // nothing... you just access another part of the array
                 // oh ic kk
  // oh, you mean outside chunk? that's why we have the first "if". we don't
  // access that
  if (x < 15)
    if (data[bindex(x + 1, y, z)] != 0)
      out &= ~4; // right
  ...
  if ((x == 16) && (y == 0) && (z == 0)) {
    printf(" curr : %i, x + 1: %i, y + 1: %i, z + 1: %i", data[bindex(x, y, z)],
           data[bindex(x + 1, y, z)], data[bindex(x, y + 1, z)],
           data[bindex(x, y, z + 1)]);
...
void register_buffer(unsigned int buffer) {
  ...
  buffers.push_back(
      std::pair<unsigned int, cudaGraphicsResource_t>(buffer, resource));
}
...
void cube_facemask(unsigned int blockbuffer) {
  void *pointer;
  size_t size;
  cudaGraphicsResourceGetMappedPointer(&pointer, &size,
                                       get_resource(blockbuffer));
  cudaError_t err;
